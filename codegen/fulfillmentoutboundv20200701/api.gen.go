...
type DeliveryDocument struct {
	// DocumentType The delivery document type. Values are `SIGNATURE` and `DELIVERY_IMAGE`.
	DocumentType string `json:"documentType"`

	// Url A URL that you can use to download the document. This URL has a `Content-Type` header. Note that the URL expires after one hour. To get a new URL, you must call the API again.
...
type DeliveryMessage struct {
	// Locale The locale for the message (for example, en_US).
...
type DropOffLocation struct {
	// Attributes Additional information about the drop-off location that can vary depending on the type of drop-off location specified in the `type` field.
	...
	// Type Specifies the preferred location to leave the package at the destination address.
	Type DropOffLocationType `json:"type"`
}

// DropOffLocationType Specifies the preferred location to leave the package at the destination address.
...
type GetFulfillmentPreviewRequest struct {
	...
	// ShippingSpeedCategories ShippingSpeedCategory List
...
// Origin The origin for the delivery offer.
type Origin struct {
	// CountryCode The two digit country code the items should ship from. In ISO 3166-1 alpha-2 format.
	CountryCode string `json:"countryCode"`
}
