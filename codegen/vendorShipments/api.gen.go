...
// Defines values for ItemDetailsLotNumberSourceType.
const (
	FFRN  ItemDetailsLotNumberSourceType = "FFRN"
	GLN   ItemDetailsLotNumberSourceType = "GLN"
	URL   ItemDetailsLotNumberSourceType = "URL"
	USDAE ItemDetailsLotNumberSourceType = "USDA_E"
)
...
type ItemDetails struct {
	// CountryOfOrigin The two-character country code for the country where the product was manufactured or originates. Use ISO 3166-1 alpha-2 format.
	...
	// LotNumberSourceReference The location identifier where the product receives a traceability lot number. Provide this field for products subject to the FDA Food Safety Modernization Act (FSMA) Section 204. When you provide `lotNumberSourceReference`, you must also specify the corresponding `lotNumberSourceType` field.
	LotNumberSourceReference *string `json:"lotNumberSourceReference,omitempty"`

	// LotNumberSourceType The identifier type used for the lot number source. Provide this field when you specify `lotNumberSourceReference`.
	LotNumberSourceType *ItemDetailsLotNumberSourceType `json:"lotNumberSourceType,omitempty"`
...
// ItemDetailsLotNumberSourceType The identifier type used for the lot number source. Provide this field when you specify `lotNumberSourceReference`.
type ItemDetailsLotNumberSourceType string
...
type RegulationReferences struct {
	// DueDiligenceExemptionCode The exemption code for EUDR products exempt from due diligence requirements.
	DueDiligenceExemptionCode *string `json:"dueDiligenceExemptionCode,omitempty"`

	// DueDiligenceInformation The EUDR Due Diligence information pre-loaded in Vendor Central.
	DueDiligenceInformation *string `json:"dueDiligenceInformation,omitempty"`

	// DueDiligenceReference The reference number from the vendor's EUDR Due Diligence Statement (DDS) submitted to the EU Commission portal or provided to Amazon through ONIX feed.
	DueDiligenceReference *string `json:"dueDiligenceReference,omitempty"`

	// DueDiligenceVerification The EUDR Due Diligence verification number provided by the EU Commission for the submitted DDS.
...
type Shipment struct {
	...
	// ShipmentFreightTerm Specifies if payment is Collect (WePay) or Prepaid (TheyPay). Required.
	...
	// TransactionType Indicates the type of transportation request (for example, `New` or `Cancel`). Each `transactionType` has a unique set of operations and there are corresponding details to be populated for each operation.
...
// ShipmentShipmentFreightTerm Specifies if payment is Collect (WePay) or Prepaid (TheyPay). Required.
type ShipmentShipmentFreightTerm string

// ShipmentTransactionType Indicates the type of transportation request (for example, `New` or `Cancel`). Each `transactionType` has a unique set of operations and there are corresponding details to be populated for each operation.
...
type GetShipmentDetailsParams struct {
	...
	// EstimatedDeliveryAfter Get Shipment Details by passing Estimated Delivery Date After. Must be in <a href='https://developer-docs.amazon.com/sp-api/docs/iso-8601'>ISO 8601</a> format.
...
type GetShipmentLabelsParams struct {
	...
	// NextToken A token that you use to retrieve the next page of results. The response includes `nextToken` when the number of results exceeds the specified `pageSize` value. To get the next page of results, call the operation with this token and include the same arguments as the call that produced the token. To get a complete list, call this operation until `nextToken` is null. Note that this operation can return empty pages.
	NextToken *string `form:"nextToken,omitempty" json:"nextToken,omitempty"`
