...
type DeliveryWindowOption struct {
	// AvailabilityType The type of delivery window availability. Values: `AVAILABLE`, `BLOCKED`, `CONGESTED`, `DISCOUNTED`
...
type Item struct {
	...
	// LabelOwner Specifies who will label the items. Options include `AMAZON`, `SELLER`, and `NONE`. `AMAZON` is not an accepted value in the US marketplace.
...
type ShipmentDestination struct {
	...
	// WarehouseId The warehouse that the shipment should be sent to. This can be empty if the destination type is `AMAZON_OPTIMIZED`.
	WarehouseId *string `json:"warehouseId,omitempty"`
}
