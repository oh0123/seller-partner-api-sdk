...
// Defines values for AmountUnitOfMeasure.
const (
	Eaches AmountUnitOfMeasure = "Eaches"
)
...
// Defines values for DropOffLocationType.
const (
	DropOffLocationTypeBICYCLEBASKET            DropOffLocationType = "BICYCLE_BASKET"
	DropOffLocationTypeDELIVERYBOX              DropOffLocationType = "DELIVERY_BOX"
	DropOffLocationTypeDONOTLEAVEUNATTENDED     DropOffLocationType = "DO_NOT_LEAVE_UNATTENDED"
	DropOffLocationTypeFALLBACKNEIGHBORDELIVERY DropOffLocationType = "FALLBACK_NEIGHBOR_DELIVERY"
	DropOffLocationTypeFRONTDOOR                DropOffLocationType = "FRONT_DOOR"
	DropOffLocationTypeGARAGE                   DropOffLocationType = "GARAGE"
	DropOffLocationTypeGASMETERBOX              DropOffLocationType = "GAS_METER_BOX"
	DropOffLocationTypeRECEPTIONIST             DropOffLocationType = "RECEPTIONIST"
)
...
type Amount struct {
	// UnitOfMeasure The unit of measure for the amount.
	UnitOfMeasure AmountUnitOfMeasure `json:"unitOfMeasure"`
...
// AmountUnitOfMeasure The unit of measure for the amount.
type AmountUnitOfMeasure string
...
type CreateFulfillmentOrderItem struct {
	...
	// SellerFulfillmentOrderItemId A fulfillment order item identifier that the seller creates to track fulfillment order items. Used to disambiguate multiple fulfillment items that have the same `sellerSku` value. For example, the seller might assign different `sellerFulfillmentOrderItemId` values to two items in a fulfillment order that share the same `sellerSku` value but have different `giftMessage` values.
...
type CreateFulfillmentOrderRequest struct {
	...
	// DisplayableOrderDate Date timestamp
...
type CreateFulfillmentReturnResponse struct {
	...
	// Payload The result for the `createFulfillmentReturn` operation.
...
type CreateReturnItem struct {
	...
	// SellerReturnItemId An identifier the seller assigns to the return item.
...
type DeliveryDocument struct {
	// DocumentType The delivery document type. The only possible value is `DELIVERY_IMAGE`.
	DocumentType string `json:"documentType"`

	// Url A URL that is valid for one hour to download the document. In case of URL expiry, call the API again to get a new url. The URL will have a Content-Type header.
...
type DeliveryMessage struct {
	// Locale The locale for the message (e.g., `en_US`).
	Locale *string `json:"locale,omitempty"`

	// Text The message content for a delivery offering.
...
type DeliveryOffer struct {
	...
	// Policy The policy for a delivery offering.
...
type DeliveryPolicy struct {
	// Message Localized messaging for a delivery offering.
...
type DeliveryPreferences struct {
	// DeliveryInstructions Additional delivery instructions. For example, this could be instructions on how to enter a building, nearby landmark or navigation instructions, or `Beware of dogs`.
...
type Destination struct {
	// DeliveryAddress A physical address with varying degrees of precision. A more precise address can provide more accurate results than country code and postal code alone.
...
type DropOffLocation struct {
	// Attributes Additional information about the drop-off location. This information can vary depending on the type of drop-off location specified in the `type` field.
	//
	...
	// Type The drop-off location type at the destination address.
	Type DropOffLocationType `json:"type"`
}

// DropOffLocationType The drop-off location type at the destination address.
type DropOffLocationType string
...
type FeatureSettings struct {
	...
	// FeatureName The name of the feature. Valid feature names are:
	//
...
type FulfillmentOrderItem struct {
	...
	// EstimatedShipDate Date timestamp
...
type FulfillmentPreview struct {
	...
	// OrderUnfulfillableReasons String list
...
type FulfillmentPreviewShipment struct {
	// EarliestArrivalDate Date timestamp
	EarliestArrivalDate *Timestamp `json:"earliestArrivalDate,omitempty"`

	// EarliestShipDate Date timestamp
	...
	// LatestArrivalDate Date timestamp
	LatestArrivalDate *Timestamp `json:"latestArrivalDate,omitempty"`

	// LatestShipDate Date timestamp
...
type FulfillmentShipment struct {
	...
	// FulfillmentCenterId An identifier for the fulfillment center from which the shipment is sent.
	...
	// ShippingDate Date timestamp
	ShippingDate *Timestamp `json:"shippingDate,omitempty"`

	// ShippingNotes Provides additional insight into shipment timeline. Primarily used to communicate that actual delivery dates aren't available.
	ShippingNotes *[]string `json:"shippingNotes,omitempty"`
}
...
type FulfillmentShipmentItem struct {
	// ManufacturerLotCodes String list
...
type FulfillmentShipmentPackage struct {
	// AmazonFulfillmentTrackingNumber The Amazon fulfillment tracking number, if provided, can be used to obtain tracking and delivery information.
...
type GetDeliveryOffersRequest struct {
	// Product The product details for the delivery offer.
	Product GetDeliveryOffersProduct `json:"product"`

	// Terms The delivery terms for the delivery offer.
...
type GetDeliveryOffersResponse struct {
	...
	// Payload A list of delivery offers, including offer expiration, earliest and latest date and time range, and the delivery offer policy.
...
type GetDeliveryOffersResult struct {
	// DeliveryOffers An array of delivery offer information.
...
type GetFeatureInventoryResponse struct {
	...
	// Payload The payload for the `getEligibleInventory` operation.
...
type GetFeatureSkuResult struct {
	...
	// IneligibleReasons A list of one or more reasons that the seller SKU is ineligible for the feature.
	//
	// Possible values:
...
type GetFulfillmentOrderResponse struct {
	...
	// Payload The request for the `getFulfillmentOrder` operation.
...
type GetFulfillmentPreviewRequest struct {
	...
	// IncludeDeliveryWindows When true, returns the `ScheduledDeliveryInfo` response object, which contains the available delivery windows for a Scheduled Delivery. The `ScheduledDeliveryInfo` response object can only be returned for fulfillment order previews with `ShippingSpeedCategories` equal to `ScheduledDelivery`.
	...
	// ShippingSpeedCategories List of shipping speed categories.
...
type GetPackageTrackingDetailsResponse struct {
	...
	// Payload Tracking details of package
...
type ListAllFulfillmentOrdersResponse struct {
	...
	// Payload The request for the `listAllFulfillmentOrders` operation.
...
type ListReturnReasonCodesResponse struct {
	...
	// Payload The request for the `listReturnReasonCodes` operation.
...
type LockerDetails struct {
	// LockerAccessCode Indicates the locker access code
	LockerAccessCode *string `json:"lockerAccessCode,omitempty"`

	// LockerNumber Indicates the locker number
...
// Origin The origin for the delivery offer.
type Origin struct {
	// CountryCode The two-digit country code (in ISO 3166-1 alpha-2 format) for the country where the items will ship from.
	CountryCode string `json:"countryCode"`
}
...
type ProductIdentifier struct {
	// MerchantSku The merchant SKU for the product.
...
type ReturnAuthorization struct {
	...
	// FulfillmentCenterId An identifier for the Amazon fulfillment center to which the return items should be sent.
	FulfillmentCenterId string `json:"fulfillmentCenterId"`
...
type ReturnItem struct {
	...
	// StatusChangedDate Date timestamp
...
type UnfulfillablePreviewItem struct {
	// ItemUnfulfillableReasons String list
...
type VariablePrecisionAddress struct {
	...
	// City The city where the person, business, or institution is located. This property should not be used in Japan.
	City *string `json:"city,omitempty"`
...
type GetFeatureInventoryParams struct {
	...
	// QueryStartDate A date that you can use to select inventory that has been updated since a specified date. An update is defined as any change in feature-enabled inventory availability. The date must be in the format `yyyy-MM-ddTHH:mm:ss.sssZ`
...
type GetPackageTrackingDetailsParams struct {
	// PackageNumber The unencrypted package identifier. You can obtain this value from the `getFulfillmentOrder` operation.
	PackageNumber int32 `form:"packageNumber" json:"packageNumber"`
}

// DeliveryOffersJSONRequestBody defines body for DeliveryOffers for application/json ContentType.
type DeliveryOffersJSONRequestBody = GetDeliveryOffersRequest
...
type ClientInterface interface {
	// DeliveryOffersWithBody request with any body
	DeliveryOffersWithBody(ctx context.Context, contentType string, body io.Reader) (*http.Response, error)

	DeliveryOffers(ctx context.Context, body DeliveryOffersJSONRequestBody) (*http.Response, error)
...
func (c *Client) DeliveryOffersWithBody(ctx context.Context, contentType string, body io.Reader) (*http.Response, error) {
	req, err := NewDeliveryOffersRequestWithBody(c.Server, contentType, body)
	if err != nil {
		return nil, err
	}
...
func (c *Client) DeliveryOffers(ctx context.Context, body DeliveryOffersJSONRequestBody) (*http.Response, error) {
	req, err := NewDeliveryOffersRequest(c.Server, body)
	if err != nil {
		return nil, err
	}
	req = req.WithContext(ctx)
...
func NewDeliveryOffersRequest(server string, body DeliveryOffersJSONRequestBody) (*http.Request, error) {
	...
	return NewDeliveryOffersRequestWithBody(server, "application/json", bodyReader)
}

// NewDeliveryOffersRequestWithBody generates requests for DeliveryOffers with any type of body
func NewDeliveryOffersRequestWithBody(server string, contentType string, body io.Reader) (*http.Request, error) {
	var err error
	...
	operationPath := fmt.Sprintf("/fba/outbound/2020-07-01/deliveryOffers")
	if operationPath[0] == '/' {
...
type ClientWithResponsesInterface interface {
	// DeliveryOffersWithBodyWithResponse request with any body
	DeliveryOffersWithBodyWithResponse(ctx context.Context, contentType string, body io.Reader) (*DeliveryOffersResp, error)

	DeliveryOffersWithResponse(ctx context.Context, body DeliveryOffersJSONRequestBody) (*DeliveryOffersResp, error)
...
type DeliveryOffersResp struct {
	Body         []byte // Might be nil if decoded via streaming
	HTTPResponse *http.Response
	JSON200      *GetDeliveryOffersResponse
	JSON400      *GetDeliveryOffersResponse
	JSON401      *GetDeliveryOffersResponse
	JSON403      *GetDeliveryOffersResponse
	JSON404      *GetDeliveryOffersResponse
	JSON429      *GetDeliveryOffersResponse
	JSON500      *GetDeliveryOffersResponse
	JSON503      *GetDeliveryOffersResponse
}

// Status returns HTTPResponse.Status
func (r DeliveryOffersResp) Status() string {
...
func (r DeliveryOffersResp) StatusCode() int {
	if r.HTTPResponse != nil {
...
// DeliveryOffersWithBodyWithResponse request with arbitrary body returning *DeliveryOffersResp
func (c *ClientWithResponses) DeliveryOffersWithBodyWithResponse(ctx context.Context, contentType string, body io.Reader) (*DeliveryOffersResp, error) {
	rsp, err := c.DeliveryOffersWithBody(ctx, contentType, body)
	if err != nil {
		return nil, err
	}
	return ParseDeliveryOffersResp(rsp)
}

func (c *ClientWithResponses) DeliveryOffersWithResponse(ctx context.Context, body DeliveryOffersJSONRequestBody) (*DeliveryOffersResp, error) {
	rsp, err := c.DeliveryOffers(ctx, body)
	if err != nil {
		return nil, err
	}
	return ParseDeliveryOffersResp(rsp)
...
// ParseDeliveryOffersResp parses an HTTP response from a DeliveryOffersWithResponse call
func ParseDeliveryOffersResp(rsp *http.Response) (*DeliveryOffersResp, error) {
	defer func() { _ = rsp.Body.Close() }()

	response := &DeliveryOffersResp{
		HTTPResponse: rsp,
	}
	...
	case strings.Contains(contentType, "json") && rsp.StatusCode == 200:
		var dest GetDeliveryOffersResponse
